...
File.open(arg0, "w") {|file|
	text.lines{|line|
		if last_line
			...
			if m = /^\s*proc ([a-z]+_)?([a-z]+_)?([a-z]+_)?([a-z]+_)?([a-z]+_)?([a-z]+_)?([a-z]+_)?\w*\*\(\s*(`?\w+`?):(?: ptr| var){0,2} (\w+)/.match(long)
				a, b, c, d, e, f, g, pp, dt = m[1..9]
				if a && pp && dt
					dt.sub!(/^GDBus/, 'GDbus')
					#dt.sub!(/^GIOStream/, 'GIoStream')
					dt.sub!(/^GIO/, 'GIo')

					if dt == 'GResource' &&  b == 'resources_'
						dt = 'GResources'
					end
					#	dt = 'GDbusConnection'
					#elsif dt == 'GDBusAuthObserver'
					#	dt = 'GDbusAuthObserver'
					#elsif dt == 'GDBusConnection'
					#	dt = 'GDbusConnection'
					#elsif dt == ''
				#		dt = ''
