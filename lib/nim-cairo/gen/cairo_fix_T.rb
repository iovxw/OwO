...
dep = Array.new
text.lines{|line|
	if m = /^(\s*)(\w+)_t\* = object $/.match(line)
		list << m[2]
		n = m[2].gsub(/^cairo_/, '').capitalize
		check << n
...
list.each{|pat|
	p1 = '\bptr ptr ' + pat + '_t\b'
	p2 = 'vaaaaar' + pat.gsub(/^cairo_/, '').capitalize
	sedlist << "s/#{p1}/#{modname}#{p2}/g\n"
	new.gsub!(/#{p1}/, p2)
	p1 = '\bptr ' + pat + '_t\b'
	p2 = 'ptttttr' + pat.gsub(/^cairo_/, '').capitalize
	sedlist << "s/#{p1}/#{modname}#{p2}/g\n"
	new.gsub!(/#{p1}/, p2)
	p1 = '\b' + pat + '_t\b'
	p2 = pat.gsub(/^cairo_/, '').capitalize + 'Obj'
	sedlist << "s/#{p1}/#{modname}\.#{p2}/g\n"
